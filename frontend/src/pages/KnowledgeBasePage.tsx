...
const labels: Record<string, string> = {
  uploaded: '等待处理', processing: '解析中', indexed: '已入库', failed: '失败', deleted: '已删除', deduplicated: '已存在',
};
...
export default function KnowledgeBasePage() {
  ...
  return <div className="h-full overflow-y-auto bg-[#f7f8fa]"><div className="mx-auto max-w-[1240px] px-8 py-8">
    ...
    <section className="rounded-xl border border-ink-200 bg-white p-5"><div className="flex flex-wrap items-end gap-3"><div className="min-w-[250px] flex-1"><label className="mb-2 block text-xs font-medium text-ink-600">选择文档</label><input ref={picker} type="file" onChange={e => setFile(e.target.files?.[0] ?? null)} className="block w-full text-sm text-ink-600 file:mr-3 file:rounded-md file:border-0 file:bg-ink-100 file:px-3 file:py-2 file:text-sm file:text-ink-700" /></div><div className="min-w-[220px] flex-1"><label className="mb-2 block text-xs font-medium text-ink-600">标题（可选）</label><input value={title} onChange={e => setTitle(e.target.value)} placeholder={file?.name || '默认使用文件名'} className="w-full rounded-lg border border-ink-200 px-3 py-2 text-sm outline-none focus:border-accent-500" /></div><button disabled={!file || uploading} onClick={() => void upload()} className="rounded-lg bg-ink-900 px-4 py-2 text-sm font-medium text-white disabled:cursor-not-allowed disabled:opacity-40">{uploading ? '上传中…' : '上传并入库'}</button></div><p className="mt-3 text-xs text-ink-400">文件选择器展示全部文件；服务端仅接受 PDF、DOCX、TXT、Markdown（扩展名不区分大小写），当前请求大小上限 5MB。</p></section>
    <section className="mt-5 overflow-hidden rounded-xl border border-ink-200 bg-white"><div className="flex items-center justify-between border-b border-ink-100 px-5 py-4"><div><h2 className="text-sm font-semibold text-ink-900">文档列表</h2><p className="mt-1 text-xs text-ink-500">失败任务可从 OSS 重新读取并处理。</p></div><span className="text-xs text-ink-400">{documents.length} 个文档</span></div>{loading ? <div className="px-5 py-12 text-center text-sm text-ink-500">正在加载…</div> : <div className="divide-y divide-ink-100">{documents.map(document => <div key={document.id} className="flex flex-wrap items-center gap-4 px-5 py-4"><div className="min-w-[260px] flex-1"><div className="font-medium text-ink-800">{document.title}</div><div className="mt-1 text-xs text-ink-400">{document.filename} · {formatSize(document.sizeBytes)} · {document.chunkCount} 个分块</div>{document.error && <div className="mt-1 text-xs text-rose-600">{document.error}</div>}</div><span className={`rounded-full px-2.5 py-1 text-xs ${tone(document.status)}`}>{labels[document.status] || document.status}</span><div className="flex gap-3 text-xs">{document.status === 'failed' && <button disabled={busy === document.id} onClick={() => void action(document, 'retry')} className="text-accent-700 hover:underline disabled:opacity-40">重新处理</button>}{document.status !== 'deleted' && <button disabled={busy === document.id} onClick={() => void action(document, 'soft-delete')} className="text-rose-700 hover:underline disabled:opacity-40">删除</button>}</div></div>)}{!documents.length && <div className="px-5 py-12 text-center text-sm text-ink-400">还没有上传知识文档</div>}</div>}</section>
  </div></div>;
}
