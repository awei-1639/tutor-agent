...
export default function ChatPage() {
  ...
  function send() {
    ...
    const ctrl = streamChat(
      { conversationId: convId, message: text },
      {
        ...
        onDone: () => { setStreaming(false); setStage(null); },
        onError: msg => {
  ...
  return (
    <div className="h-full flex flex-col" onMouseOver={onMouseOver} onMouseMove={e => hoverCite && setTooltipPos({ x: e.clientX, y: e.clientY })}>
      ...
      <div className="border-t border-ink-100 bg-white px-6 py-4">
        <div className="max-w-3xl mx-auto flex gap-3">
          ...
          />
          {streaming ? (
            <button onClick={stop} className="px-5 py-3 bg-ink-200 hover:bg-ink-300 text-ink-700 rounded-md font-medium">
              停止
            </button>
          ) : (
            <button onClick={send} disabled={!input.trim()} className="px-5 py-3 bg-accent-500 hover:bg-accent-600 disabled:bg-ink-200 text-white rounded-md font-medium">
              发送
            </button>
          )}
        </div>
