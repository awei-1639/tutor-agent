...
final class RetrievalJudgePromptFactory {
    List<ChatMessage> forNodeIds(String query, List<String> nodeIds) {
        String prompt = "查询: " + query + "\n已累积证据节点ID: " + nodeIds + "\n证据正文未提供时只能作保守判断。"
                + "\n请判断这些证据是否足以完整回答查询。输出JSON {sufficient: bool, followup_query: string|null, missing: string|null}";
        return List.of(SystemMessage.from("你是多跳检索的证据充分性判断器。判断规则: "
                        + "1) 若证据覆盖了回答问题所需的所有关键概念/前置技能/资源→sufficient=true; "
                        + "2) 否则→sufficient=false, followup_query=针对缺口的更窄查询, missing=缺失的关键概念关键词; "
                        + "3) followup_query 不应与原 query 重复, 应聚焦缺失的具体子概念。"), UserMessage.from(prompt));
    }

    List<ChatMessage> forEvidence(String query, List<Evidence> evidence) {
        return forEvidence(query, query, evidence);
    }

    List<ChatMessage> forEvidence(String originalQuery, String currentSubQuery, List<Evidence> evidence) {
        ...
        return List.of(SystemMessage.from("你是多跳检索的证据充分性判断器。必须依据证据正文和图路径判断，不能仅因节点名称相似就判定充分。"
                        + "若不充分，followup_query 只能针对当前子问题的明确缺口，长度不超过 240 个字符，不得重复原查询或改变主题；"
                        + "若充分则 followup_query=null。"), UserMessage.from(prompt.toString()));
    }
