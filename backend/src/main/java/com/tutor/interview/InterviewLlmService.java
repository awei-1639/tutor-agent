...
public class InterviewLlmService {
    ...
    public InterviewLlmService(LlmGateway gateway) {
        ...
        try {
            String json = gateway.chatJson(Purpose.PLAN, List.of(
                    ...
                    UserMessage.from("岗位: " + targetRole + "\n主题: " + topic + "\nJD: " + excerpt(jobDescription, 2000)
                            + "\n面试类型: " + interviewType + "\n难度: " + difficulty + "\n考察维度: " + dimension
                            + "\n第 " + number + " 题\n已考题（必须避免语义重复）: "
                            + String.join("；", priorQuestions.stream().map(this::shorten).toList()))), traceId);
            JsonNode node = mapper.readTree(json);
            String question = clip(node.path("question").asText().replaceAll("[\\n\\r]", " ").trim(), 160);
            List<String> required = textList(node.path("required_points"));
            List<String> bonus = textList(node.path("bonus_points"));
            List<String> critical = textList(node.path("critical_errors"));
            if (!question.isBlank() && InterviewScoringQuality.validContract(required, bonus, critical)) {
                return new InterviewSession.QuestionSpec(question, dimension, required, bonus, critical);
            }
    ...
    String generateFollowUp(String question, String answer, List<String> missingPoints, String traceId) {
        try {
            String responseJson = gateway.chatJson(Purpose.JUDGE, List.of(
                    ...
                    UserMessage.from("{\"question\":\"" + jsonEscape(question) + "\",\"candidate_answer\":\""
                            + jsonEscape(answer) + "\",\"missing_points\":" + jsonArray(missingPoints) + "}")), traceId);
            String followUp = mapper.readTree(responseJson).path("follow_up").asText();
            if (followUp.isBlank()) throw new IllegalArgumentException("missing follow_up");
            return clip(followUp.replaceAll("[\\n\\r]", " ").trim(), 300);
        } catch (Exception ignored) {
        ...
        try {
            String json = gateway.chatJson(Purpose.JUDGE, List.of(
                    ...
                    UserMessage.from("{\"question\":\"" + jsonEscape(question) + "\",\"assessment_contract\":"
                            + validJson(assessmentContract) + ",\"candidate_answer\":\"" + jsonEscape(answer) + "\"}")), traceId);
            JsonNode node = mapper.readTree(json);
            int score = requiredScore(node.path("score"));
            double confidence = requiredConfidence(node.path("confidence"));
            List<String> evidenceQuotes = textList(node.path("evidence_quotes"));
            if (evidenceQuotes.isEmpty() || evidenceQuotes.stream().anyMatch(quote -> !answer.contains(quote))) {
                throw new IllegalArgumentException("score evidence is not grounded in the answer");
            }
            return new InterviewSession.Scorecard(score, textList(node.path("strengths")), textList(node.path("missing_points")), confidence,
