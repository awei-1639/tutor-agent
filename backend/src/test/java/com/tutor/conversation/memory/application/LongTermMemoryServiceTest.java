...
class LongTermMemoryServiceTest {
    ...
    void mergesLocalAndRemoteMemoriesWithoutDuplicateSummaries() {
        ...
        LongTermMemoryService service = new LongTermMemoryService(local, mem0, consent, breaker, transactions, outbox, mock(com.tutor.conversation.memory.local.FactStore.class), 30);
    ...
    void ordersUniqueMemoriesByGlobalRelevanceBeforeApplyingLimit() {
        ...
        LongTermMemoryService.RecallResult result = new LongTermMemoryService(
                local, mem0, consent, breaker, transactionTemplate(), outbox, mock(com.tutor.conversation.memory.local.FactStore.class), 30)
    ...
    void remoteFailureFallsBackToLocalMemory() {
        ...
        LongTermMemoryService.RecallResult result = new LongTermMemoryService(local, mem0, consent, breaker, transactions, outbox, mock(com.tutor.conversation.memory.local.FactStore.class), 30)
                .recall(7L, "query", "trace");
    ...
    void doesNotSendUnreviewedConversationToRemoteMemory() {
        ...
        when(consent.enabledFor(7L)).thenReturn(true);
        when(breaker.allowRequest()).thenReturn(true);
        new LongTermMemoryService(local, mem0, consent, breaker, transactions, outbox, mock(com.tutor.conversation.memory.local.FactStore.class), 30).remember(7L, "question", "answer", "trace");
    ...
    void queuesDiscoveryDeleteWhenRemoteUuidHasNotBeenObserved() {
        ...
        LongTermMemoryService service = new LongTermMemoryService(
                local, mem0, consent, breaker, transactions, outbox, mock(com.tutor.conversation.memory.local.FactStore.class), 30);
